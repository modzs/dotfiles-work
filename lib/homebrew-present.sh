...
# The directories the privileged step CREATES, and which have to belong to the
# account this configuration is set up for if they are there at all. Homebrew's
# own list of what its installer creates, minus the prefix directory itself:
# that one is left root-owned on purpose - `install -d -o root -g wheel` is what
# creates it - and it is the contents that have to be the user's.
#
# The one list, and lib/homebrew-initialize-prefix.sh creates exactly it, so
# there is no second copy to drift.
#
# This is NOT the list the marker promises, and the difference is load-bearing -
# see dotfiles_homebrew_unusable. Creating a directory is not promising it will
# still be there: Homebrew removes most of these itself once they are empty.
...
#
# A path that is NOT there counts only once the marker is present, and only for
# the two paths the marker actually promises: $prefix/bin and the library. Both
# halves of that are load-bearing.
#
# Before the marker nothing has been promised, so a prefix with none of these
# directories is simply one the privileged step has not reached yet. Reading it
# the other way round turns every first bootstrap into a refusal; not reading it
# at all is what let a prefix whose bin had been deleted go on calling itself
# managed, with bootstrap.sh skipping the only step that could rebuild it and
# every switch afterwards dying inside `ln`.
#
# And the promise is those two paths and no others because they are the only
# ones dotfiles_homebrew_prefix_link writes into - which is what makes its `ln`
# unable to fail - while the rest are directories HOMEBREW ITSELF DELETES as a
# matter of routine. In the pinned source, `Keg.must_exist_subdirectories`
# (Library/Homebrew/keg.rb) is `bin etc include lib sbin share opt
# var/homebrew/linked`; `share/zsh` and `share/zsh/site-functions` are not on
# it, so `Keg#unlink` rmdirs them once they are empty, and
# `Cleanup#prune_prefix_symlinks_and_directories` removes them with no user
# action at all. Promising the whole creation list therefore made a plain
# `brew uninstall gh` lock the prefix out permanently, with no escape but
# deleting the marker and re-running bootstrap for another password - which is
# the one promise this repository cannot break. Do not widen this back for
# symmetry with the ownership half below; they are different questions.
#
...
#   unusable  the prefix cannot be given to this account - it holds a directory
#             that is not the account's, or it carries the marker and has lost
#             one of the two paths the marker promises. Also the run stops.
#   managed   the marker is there AND $prefix/bin and the library are there and
#             are this account's, so the privileged setup has already run and
#             must not run again.
...
# them. Exactly that, and deliberately nothing else - those two are what
# dotfiles_homebrew_prefix_link writes into, which is what makes its `ln` unable
# to fail the way it once did, and every other directory in the prefix belongs
# to Homebrew, which prunes them. See dotfiles_homebrew_unusable.
...
dotfiles_homebrew_report_unusable() {
  ...
  local paths missing="" present=""

  # Asked once. The two kinds of path have different fixes, and a message that
  # offered both to a user with only one of them would be telling them to delete
  # a marker file that is not there.
  paths=$(dotfiles_homebrew_unusable "$prefix" "$library" "$uid")
  while IFS= read -r path; do
    [ -n "$path" ] || continue
    if [ -e "$path" ]; then
      present=yes
...
PATHS
  ...
  echo "${indent}$prefix cannot be given to your account." >&2
  if [ -n "$missing" ] && [ -n "$present" ]; then
    echo "${indent}These paths are either missing or somebody else's:" >&2
  elif [ -n "$missing" ]; then
    echo "${indent}These paths are missing:" >&2
  else
    echo "${indent}These paths are somebody else's:" >&2
  fi
  while IFS= read -r path; do
    [ -n "$path" ] || continue
    if [ -e "$path" ]; then
...
PATHS
  ...
  echo "${indent}" >&2
  echo "${indent}Nothing has been changed." >&2
  ...
  if [ -n "$present" ]; then
    echo "${indent}" >&2
    echo "${indent}This repository creates the directories Homebrew needs and" >&2
    echo "${indent}gives those to you; it never takes over one that was already" >&2
    echo "${indent}somebody else's - on a Mac you share with software you did" >&2
    echo "${indent}not install, reassigning a directory out from under it is" >&2
    echo "${indent}not a setup script's decision. Give those paths to your own" >&2
    echo "${indent}account first - HOW-TO.md says how - and then run" >&2
    echo "${indent}./bootstrap.sh again." >&2
  fi
}
...
dotfiles_homebrew_preflight() {
  ...
  #
  # Everything below judges $prefix alone, which is the right question for every
  # step that writes - and the wrong question to open with. An Apple silicon Mac
  # carrying an Intel Homebrew at /usr/local, with Homebrew's own
  # `eval "$(/usr/local/bin/brew shellenv)"` line in its profile, has a Homebrew;
  # a preflight that inspected /opt/homebrew alone announced "no Homebrew in it"
  # and carried on. The password then bought a prefix nothing used, because the
  # Brewfile step follows HOMEBREW_PREFIX and handed the whole list to the other
  # Homebrew - silently, and into a prefix this repository does not manage.
  #
  # So: if this machine already has a usable Homebrew that is not ours, name it
