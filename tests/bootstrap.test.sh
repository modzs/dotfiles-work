...
#
# Everything here is about the interactive steps, which live in lib/personalize.sh
# so they can be driven by a script instead of a person. bootstrap.sh itself is
# not run end to end by anything: running it installs Nix and asks for a password
# twice. Its step ORDER is deliberately not asserted here either - AGENTS.md says
# why, and why a grep over its source is not an acceptable substitute.
#
